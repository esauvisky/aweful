...
if __name__ == "__main__":
    ...
    # train
    callback = EarlyStopping(monitor='val_accuracy', patience=5, min_delta=0.1)
    ...
    for i in range(len(convlstm_model_predictions)):
        if convlstm_model_predictions[i] != y_test[i]:
            logger.warning(f"This was predicted {'sleep' if convlstm_model_predictions[i] else 'not sleep'} and is actually {'sleep' if y_test[i] else 'not sleep'}")
            image = image_utils.array_to_img(X_test[i][0])
            image.show()
            input()
        else:
